...
- **완료 조건**
  - [x] 서버가 지정 포트로 기동되고 헬스 확인용 응답이 반환된다
  - [x] 프론트 오리진에서의 요청만 CORS 허용된다
  - [x] 라우트에서 던진 에러가 errorHandler 하나를 통해 일관된 상태 코드/메시지로 반환된다
...
- **완료 조건**
  - [ ] 신청 성공 시 "신청 완료" 상태와 취소 버튼으로 화면이 전환된다
  - [ ] 신청 취소 성공 시 다시 신청 가능 상태로 화면이 갱신된다
  - [ ] 신청 종료일이 지난 프로모션의 신청 버튼이 비활성화된다
  - [ ] 정원이 찬 프로모션의 신청 버튼이 비활성화된다
  - [ ] 서버가 중복 신청 또는 정원 마감으로 거부하면 각각의 안내가 노출된다
  - [ ] 신청/취소 후 현재 신청 인원이 화면에 즉시 반영된다
...
- **완료 조건**
  - [ ] 프로모션 제목, 모집 인원, 현재 신청 인원이 표시된다
  - [ ] 신청자 목록이 이름·신청 일시 테이블로 표시된다
  - [ ] 신청자가 없으면 "신청자 없음"이 노출된다
  - [ ] 타인 등록 프로모션 접근 시 권한 없음으로 처리된다
...
- **완료 조건**
  - [ ] 6개 화면 모두 모바일 폭에서 가로 스크롤 없이 표시된다
  - [ ] 목록 카드가 모바일에서 전체 폭을 사용하고 등록 버튼이 헤더 아래로 배치된다
  - [ ] 폼 화면에서 신청 시작일/종료일이 모바일에서 세로로 쌓인다
  - [ ] 신청 현황의 신청자 테이블이 모바일에서 카드 형태로 표시된다
